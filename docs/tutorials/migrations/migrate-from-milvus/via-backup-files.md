...
- You have made necessary preparations for migration based on the migration method:
    ...
    - **From Volume**: 
        ...
        - **External volume**: If your backup files are in a cloud object storage bucket, create an [external volume](./external-volume) that maps to that bucket. You can then migrate data directly from the external volume without providing credentials each time.
...
With backup files ready, you can migrate the data from local files, an object storage, or a volume or an object storage.
