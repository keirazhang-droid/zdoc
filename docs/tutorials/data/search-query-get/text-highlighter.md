...
## Search term highlighting in BM25 full text search\{#search-term-highlighting-in-bm25-full-text-search}

When you perform a BM25 full text search, you can highlight the **search terms** in the returned result to help explain why a document matches the query. To learn more about BM25 full text search, refer to [Full Text Search](./full-text-search).
...
When highlighting is enabled, Zilliz Cloud returns highlighted text in a dedicated `highlight` field. By default, highlighted output is returned as a fragment starting from the first matched term.
...
## Query term highlighting in filtering\{#query-term-highlighting-in-filtering}
...
When query term highlighting is enabled for filtering, Zilliz Cloud returns highlighted text in a dedicated `highlight` field. By default, highlighted output is returned as a fragment starting from the first matched term.
...
By default, Zilliz Cloud returns highlighted text as fragments starting from the first matched term. Fragment-related settings allow you to further control how fragments are returned, without changing which terms are highlighted.
...
With fragment-based highlighting enabled, Zilliz Cloud returns highlighted text as fragments in the `highlight` field:
...
### Example 2: Highlight query terms in filtering\{#example-2-highlight-query-terms-in-filtering}
...
### Example 3: Return highlights as fragments\{#example-3-return-highlights-as-fragments}
...
### Example 4: Multi-query highlighting\{#example-4-multi-query-highlighting}
