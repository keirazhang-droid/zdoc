...
beta: FALSE
notebook: FALSE
description: "製品カタログ、コンテンツ管理システム、ユーザーの嗜好エンジンなどのアプリケーションを構築する際、ベクトル埋め込みとともに柔軟なメタデータを保存する必要があることがよくあります。製品属性はカテゴリごとに異なり、ユーザーの嗜好は時間とともに変化し、ドキュメントのプロパティは複雑な入れ子構造を持っています。Zilliz Cloud の JSON フィールドは、この課題を解決するために、パフォーマンスを犠牲にすることなく柔軟な構造化データの保存とクエリを可能にします。"
type: origin
