...
beta: FALSE
notebook: FALSE
description: "地理情報システム（GIS）、マッピングツール、または位置情報サービスなどのアプリケーションを構築する際、ジオメトリデータを保存およびクエリする必要がよくあります。Milvusの`GEOMETRY`データ型は、柔軟なジオメトリデータを保存およびクエリするネイティブな方法を提供することで、この課題を解決します。 | Cloud"
type: origin
