...
beta: FALSE
notebook: FALSE
description: "タイムゾーンをまたいだ時間を追跡するアプリケーション（例：eコマースシステム、コラボレーションツール、分散ログなど）では、タイムゾーンを含むタイムスタンプを正確に処理する必要があります。Zilliz Cloud の TIMESTAMPTZ データ型は、タイムスタンプとそれに関連するタイムゾーンを保存することで、この機能を提供します。| Cloud"
type: origin
...
以下の例では、コレクションに 8,193 行のサンプルデータを挿入します。各行には以下が含まれます。
...
`TIMESTAMPTZ` はスカラー比較、期間演算、および時間コンポーネントの抽出をサポートしています。

`TIMESTAMPTZ` フィールドに対してフィルタリング操作を実行する前に、次のことを確認してください:
...
次のような算術演算子を使用します: `==`, `!=`, `<`, `>`, `<=`, `>=`。Zilliz Cloud で使用可能な算術演算子の完全なリストについては、[算術演算子](./basic-filtering-operators#arithmetic-operators) を参照してください。

<Admonition type="info" icon="📘" title="Notes">

チェーンされた範囲式 (例: `lower_bound < tsz < upper_bound`) はサポートされていません。

代わりに論理積を使用してください: `tsz > lower_bound AND tsz < upper_bound`。

</Admonition>

以下の例では、タイムスタンプ (`tsz`) が **2025-01-03T00:00:00+08:00** に等しくないエンティティをフィルタリングしています:
...
- `tsz` は、スキーマで定義された `TIMESTAMPTZ` フィールド名です。
...
#### 間隔演算\{#interval-operations}

`TIMESTAMPTZ` フィールドに対して、[ISO 8601 期間形式](https://en.wikipedia.org/wiki/ISO_8601#Durations) の **INTERVAL** 値を使用して算術演算を実行できます。これにより、データをフィルタリングする際に、タイムスタンプから日、時間、分などの期間を加算または減算できます。

たとえば、次のクエリは、タイムスタンプ (`tsz`) にゼロ日を加算した結果が **2025-01-03T00:00:00+08:00** と **等しくない** エンティティをフィルタリングします:
...
`INTERVAL` 値は [ISO 8601 期間構文](https://www.w3.org/TR/xmlschema-2/#duration) に従います。例：

- `P1D` → 1 日
- `PT3H` → 3 時間
- `P2DT6H` → 2 日 6 時間

`INTERVAL` 算術をフィルター式で直接使用できます。例：

- `tsz + INTERVAL 'P3D'` → 3 日を追加
...
`TIMESTAMPTZ` フィルタリングとベクトル類似検索を組み合わせて、時間と類似性の両方で結果を絞り込むことができます。
...
高度な使い方として、異なるレベル（例：データベース、コレクション、クエリ）でタイムゾーンを管理したり、インデックスを使用して `TIMESTAMPTZ` フィールドのクエリを高速化したりできます。
...
<table>
   ...
   <tr>
     ...
     <td><p>そのコレクションのデータベースデフォルトタイムゾーン設定を上書きします</p></td>
     <td><p>中</p></td>
   </tr>
   <tr>
...
- [基本ベクトル検索](./single-vector-search#temporarily-set-a-timezone-for-a-search)

- [マルチベクトルハイブリッド検索](./hybrid-search)

### クエリの高速化\{#accelerate-queries}

デフォルトでは、インデックスのない `TIMESTAMPTZ` フィールドに対するクエリは全行のフルスキャンを実行するため、大規模データセットでは低速になる可能性があります。タイムスタンプクエリを高速化するには、`TIMESTAMPTZ` フィールドに AUTOINDEX インデックスを作成します。
