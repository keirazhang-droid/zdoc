...
sidebar_label: "ブール値 & 数値"
beta: FALSE
notebook: FALSE
description: "ブール値または数値フィールドは、ブール値または数値を格納するスカラーフィールドです。これらの値は、2つの可能な値のいずれか、または整数（整数）と小数（浮動小数点数）のいずれかになります。これらは通常、数量、測定値、または論理的または数学的に処理する必要があるデータを表すために使用されます。 | Cloud"
type: origin
