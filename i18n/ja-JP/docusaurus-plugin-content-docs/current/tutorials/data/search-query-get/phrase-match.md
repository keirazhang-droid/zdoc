...
beta: FALSE
notebook: FALSE
description: "フレーズマッチを使用すると、クエリ用語を完全なフレーズとして含むドキュメントを検索できます。デフォルトでは、単語は同じ順序で、互いに隣接して出現する必要があります。たとえば、「robotics machine learning」というクエリは、「…typical robotics machine learning models…」のようなテキストと一致します。ここで、「robotics」、「machine」、「learning」の単語が間に他の単語を挟まずに連続して出現します。 | Cloud"
type: origin
