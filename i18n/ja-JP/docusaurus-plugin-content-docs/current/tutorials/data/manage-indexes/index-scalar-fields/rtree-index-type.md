...
beta: FALSE
notebook: FALSE
description: "`RTREE` インデックスは、Zilliz Cloud の `GEOMETRY` フィールドに対するクエリを高速化するツリー構造のデータ構造です。コレクションで点、線、多角形などの幾何オブジェクトを Well-known text (WKT) 形式で保存しており、空間フィルタリングを高速化したい場合、`RTREE` が最適な選択です。"
type: origin
