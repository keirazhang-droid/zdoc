...
# 自信を持って構築し、AI アプリケーションを強化する\{#build-with-confidence-and-supercharge-your-ai-applications}

Zilliz Cloud は、セキュリティを考慮した上でベクトル検索アプリケーションのデプロイとスケーリングを簡素化する、フルマネージドの Milvus サービスを提供します。

## 基本的なベクトル検索\{#basic-vector-search}

近似最近傍（ANN）検索を実行し、クエリベクトルに最も類似したベクトルを検索します。[詳細を確認する](./single-vector-search)。
...
ベクトル検索の前にメタデータフィルタを適用して検索範囲を絞り込み、結果の関連性を向上させます。 [詳細はこちら](./filtered-search)。
...
フィールドごとに検索結果をグループ化し、より高いレベルでデータを集約し、結果の多様性を向上させます。詳細については、[こちら](./grouping-search) を参照してください。
...
テキスト、画像などを横断したマルチモーダル検索のために、複数のベクトルフィールドを組み合わせます。詳細については、[こちら](./hybrid-search) を参照してください。
...
BM25関連性スコアリングを使用したキーワードマッチングでテキストを検索し、正確な用語の取得を実現します。詳細については、[詳細情報](./full-text-search) を参照してください。
...
16,384 の制限を超える大規模な検索結果を、ページ分割された反復処理で取得します。詳細については、[こちら](./with-iterators) を参照してください。
...
ベクトル検索なしで、スカラーフィールドまたは主キーでフィルタリングしてエンティティを取得します。[詳細はこちら](./get-and-scalar-query)。
...
プロジェクトの [プラン](./select-zilliz-cloud-service-plans) を選択し、プロジェクト内に異なるデプロイメントオプションのクラスターを作成します。
...
- [Dedicated](./create-cluster#create-a-dedicated-cluster)
 ...
 [どのデプロイメントオプションを選べばよいかわからない場合は？](https://zilliz.com/pricing)
...
## コンピューティングリソースをデータへ持ち込む\{#bring-compute-resources-to-your-data}

1. ストレージ連携を設定する。

    [AWS S3 バケット](./integrate-with-aws-s3)、[Google Cloud Storage バケット](./integrate-with-gcp)、または [Microsoft Azure BLOB ストレージコンテナー](./integrate-with-azure-blob-storage) を Zilliz Cloud と連携させます。

1. [外部ボリューム](./external-volume) を作成する。
...
1. 生のテキストデータを挿入する。
    ...
    [生データを挿入](./insert-entities) します。Zilliz Cloud は取り込み時に自動的にベクトル埋め込みを生成します。

1. 生のテキストを使用して検索を実行する。

    生のクエリテキストを提供します。Zilliz Cloud はクエリを埋め込み、保存されているベクトルと比較し、最も関連性の高い結果を [返却](./single-vector-search) します。

## 他のデータインフラからの移行\{#migrate-from-other-data-infra}

1. データソースに接続する。

    Zilliz Cloud は、Pinecone、MongoDB、Qdrant、PostgreSQL など、さまざまなデータソースをサポートしています。[移行ガイド](./migrate-from-external-sources) を参照してください。
...
1. マッピングを確認する。
    ...
    移行元と移行先のデータのスキーマ間のマッピングを設定し、確認します。

## バックアップと復元\{#backup-and-restore}

1. クラスターまたはコレクションのバックアップを作成する。

    バックアップは、クラスターまたはコレクションの特定時点のコピーです。バックアップは [手動で作成](./create-snapshot) することも、[バックアップポリシーを設定](./schedule-automatic-backups) してスケジュールされたバックアップを行うこともできます。また、災害復旧能力を向上させるために [他のリージョンにバックアップをコピー](/docs/backup-to-other-regions) することもできます。

1. (オプション) バックアップをオブジェクトストレージサービスにエクスポートする。

    作成した [バックアップファイルをエクスポート](./export-backup-files) して、AWS S3 または Azure Blob Storage に保存できます。

1. データを復元する。

    予期しないシステム障害やデータ損失が発生した場合に [データを復元](./restore-from-snapshot) します。
...
- [請求](./payment-billing)
    ...
    前払い費用なしで、使用した分のみお支払いいただけます。

- [連携](./integrate-with-third-parties)

    既存のツールやワークフローと連携させます。
