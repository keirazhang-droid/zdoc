...
<Grid columnSize="2" widthRatios="20,80">
    ...
    <div>
        ...
        - この機能は、Zilliz が最近オープンソース化したセマンティックハイライトモデル ([zilliz/semantic-highlight-bilingual-v1](https://huggingface.co/zilliz/semantic-highlight-bilingual-v1)) を搭載しており、Zilliz ホスト型モデルサービスを通じてすぐに利用できる推論サポートを提供します（[ホスト型モデル](./hosted-models) を参照）。
...
<Grid columnSize="2" widthRatios="20,80">
    ...
    <div>
        ...
        - **Zilliz ホスト型モデル（プライベートプレビュー）**: Zilliz インフラストラクチャ上に完全に管理されたモデルインスタンスを直接デプロイし、ゼロのデータ転送料金で安定した高性能な推論を確保します。モデルが Zilliz Cloud 環境で実行されるため、データはプライベートネットワーク内に留まり、プライバシーの強化と超低レイテンシーを確保します。詳細については、[ホスト型モデル](./hosted-models) を参照してください。
