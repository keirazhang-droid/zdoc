...
beta: FALSE
notebook: FALSE
description: "サービングクラスターは、リアルタイムの本番サービス向けにコンピュートとストレージの両方を組み合わせた自己完結型のサーバーです。抽出・変換・ロード（ETL）パイプラインでデータをクリーニングした後、そのデータをサービングクラスターにインポートすることで、大幅なパフォーマンス向上を実現できます。 | Cloud"
type: origin
