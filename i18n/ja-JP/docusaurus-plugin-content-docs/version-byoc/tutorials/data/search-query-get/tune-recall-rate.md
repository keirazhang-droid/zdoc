...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、検索リコールとパフォーマンスのバランスを取るために、検索パラメータ `level` を導入しています。また、現在の検索の推定リコール率をユーザーに提供するための検索パラメータ `enablerecallcalculation` も提供しています。これら2つのパラメータを組み合わせて、ベクトル検索のリコール率を調整できます。 | BYOC"
type: origin
