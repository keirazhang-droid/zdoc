...
beta: FALSE
notebook: FALSE
description: "ANN 検索では、1回のクエリで取得できるエンティティの数に最大制限があり、基本的な ANN 検索だけでは大規模な検索の要求を満たせない場合があります。topK が 16,384 を超える ANN 検索リクエストでは、SearchIterator の使用を検討することをお勧めします。このセクションでは、SearchIterator の使用方法と関連する注意点を紹介します。 | BYOC"
type: origin
