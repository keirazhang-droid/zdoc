---
title: "Lexical ハイライター | BYOC"
slug: /text-highlighter
sidebar_key: text-highlighter
sidebar_label: "Lexical ハイライター"
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud のハイライターは、テキストフィールド内の一致した用語をカスタマイズ可能なタグで囲むことで注釈を付けます。ハイライト機能は、ドキュメントが一致した理由を説明し、結果の可読性を向上させ、検索や RAG アプリケーションでのリッチなレンダリングをサポートします。 | BYOC"
type: origin
...
Zilliz Cloud の ハイライター は、テキストフィールド内の一致した用語をカスタマイズ可能なタグで囲むことで注釈を付けます。ハイライトは、ドキュメントが一致する理由を説明し、結果の可読性を向上させ、検索や RAG アプリケーションでのリッチなレンダリングをサポートします。

ハイライトは、最終的な検索結果セットに対する後処理ステップとして実行されます。これは、候補の取得、フィルタリングロジック、ランキング、またはスコアリングには影響しません。

ハイライター は、3 つの独立した制御の次元を提供します。

- **強調表示される用語**

    ハイライトされた用語の出所を選択できます。たとえば、**BM25全文検索** で使用された検索用語をハイライトしたり、**テキストベースのフィルタリング式**（`TEXT_MATCH` 条件など）で指定されたクエリ用語をハイライトしたりできます。

- **ハイライトされた用語のレンダリング方法**

    各一致の前後に挿入されるタグを設定することで、ハイライト出力での一致した用語の表示方法を制御できます。たとえば、単純なマーカー `{}` や、リッチなレンダリングのための HTML タグ `<em></em>` を使用できます。

- **ハイライトされたテキストの返却方法**

    ハイライトされた結果がフラグメントとして返される方法を制御できます。これには、フラグメントの開始位置、長さ、返されるフラグメントの数などが含まれます。

以下のセクションでは、これらのシナリオについて説明します。

## BM25全文検索における検索用語のハイライト\{#search-term-highlighting-in-bm25-full-text-search}

BM25全文検索を実行する際、ドキュメントがクエリと一致する理由を説明するために、返された結果内の**検索用語**をハイライトできます。BM25全文検索の詳細については、[Full Text Search](./full-text-search) を参照してください。

このシナリオでは、ハイライトされた用語は BM25全文検索で使用された検索用語から直接取得されます。ハイライターは、これらの用語を使用して最終結果内の一致したテキストに注釈を付けます。
...
検索用語のハイライトに加えて、テキストベースのフィルタリング式で使用される用語をハイライトすることもできます。
...
デフォルトでは、Zilliz Cloud はハイライトされたテキストを、最初に一致した用語から始まるフラグメントとして返します。フラグメント関連の設定により、どの用語がハイライトされるかを変更することなく、フラグメントの返し方をさらに細かく制御できます。
