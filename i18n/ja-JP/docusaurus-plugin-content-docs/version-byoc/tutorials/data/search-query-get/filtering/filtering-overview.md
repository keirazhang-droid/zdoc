...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、データの正確なクエリを可能にする強力なフィルタリング機能を提供します。フィルター式を使用すると、特定のスカラーフィールドを対象に、さまざまな条件で検索結果を絞り込むことができます。このガイドでは、Zilliz Cloud クラスターでのフィルター式の使用方法を、クエリ操作に焦点を当てた例を交えて説明します。これらのフィルターは、検索や削除リクエストにも適用できます。 | BYOC"
type: origin
