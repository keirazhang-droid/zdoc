...
beta: FALSE
notebook: FALSE
description: "`icu` トークナイザーは、Unicode の国際化コンポーネント（ICU）オープンソースプロジェクトに基づいて構築されており、ソフトウェアの国際化のための主要なツールを提供します。ICU の単語分割アルゴリズムを使用することで、世界中のほとんどの言語でテキストを正確に単語に分割できます。 | BYOC"
type: origin
