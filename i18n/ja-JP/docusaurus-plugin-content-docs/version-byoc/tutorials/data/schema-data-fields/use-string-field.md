...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud クラスターでは、テキスト形式のスカラーデータを `VARCHAR` および `TEXT` フィールドに保存できます。このページでは、名前、タグ、カテゴリ、外部 ID などの短くて長さが決まっている文字列メタデータに適した `VARCHAR` について説明します。 | BYOC"
type: origin
