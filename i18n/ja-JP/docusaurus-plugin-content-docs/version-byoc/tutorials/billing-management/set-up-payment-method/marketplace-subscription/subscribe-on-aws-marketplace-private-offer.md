---
title: "AWS Marketplace でのプライベートオファーを購読 | BYOC"
...
beta: FALSE
notebook: FALSE
description: "AWS Marketplace のプライベートオファーは、Zilliz が組織向けに作成するカスタム購入オプションです。AWS Marketplace の製品ページに表示されている標準の価格と条件を使用するパブリックオファーとは異なり、プライベートオファーには、交渉された価格、カスタム契約条件、特定の契約期間、定義された支払いスケジュールを含めることができます。 | BYOC"
type: origin
